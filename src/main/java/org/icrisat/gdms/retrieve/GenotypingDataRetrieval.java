...
public class GenotypingDataRetrieval {
	...
	public ArrayList<String> retrieveQTLData() {
		...
		/*System.out.println("Local ...  listToReturn=:"+listToReturn);
		ArrayList<String> centralQTLs = getCentralQTLs();
		if(null != centralQTLs) {
			listToReturn.addAll(centralQTLs);
		}*/
		//System.out.println("Central + Local ...  listToReturn=:"+listToReturn);
	...
	private ArrayList<String> getLocalQTLs() {
		...
		try {
			...
			List<Qtl> listOfAllQtls =genoManager.getAllQtl(0, (int)genoManager.countAllQtl());
			
			arrayListOfQTLRetrieveDataLocal = new ArrayList<String>();
			List<Integer> QTLidsList=new ArrayList();
			
			for (Qtl qtl : listOfAllQtls) {
				QTLidsList.add(qtl.getQtlId());
			}
			List<QtlDetailElement> listOfQtlDetailElementByQtlIds=genoManager.getQtlByQtlIds(QTLidsList, 0, QTLidsList.size());
			if (null != listOfQtlDetailElementByQtlIds) {
				for (QtlDetailElement qtlDetailElement : listOfQtlDetailElementByQtlIds) {
					...
					String strQtlName=qtlDetailElement.getQtlName();
