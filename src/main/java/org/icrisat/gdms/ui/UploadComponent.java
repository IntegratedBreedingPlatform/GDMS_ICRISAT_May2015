...
public class UploadComponent extends CustomComponent implements ItemClickListener {
	...
	public VerticalLayout buildUploadDataWindow() {
		...
		if (null != _gdmsModel.getMenuItemSelected()){
			//System.out.println("_gdmsModel.getMenuItemSelected().getText():"+_gdmsModel.getMenuItemSelected().getText());
		...
		//System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  :"+_strTitleString);
		...
		if (null != _strItemSelected || false == _strItemSelected.equals("")){
			//System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^  :"+_strItemSelected);
			...
			if(_strItemSelected.equalsIgnoreCase("Haplotypes")){
				//System.out.println("    if haplotypes    ");
				
				UploadHaplotypesComponent uploadHaplotypesComponent2 = new UploadHaplotypesComponent(_mainHomePage, _strItemSelected);
				HorizontalLayout tableForHaplotypes;
				//try {
					tableForHaplotypes = uploadHaplotypesComponent2.buildTabbedComponentForHaplotypes();
					_buildRightSideUploadLayout.replaceComponent(_currentComponent, tableForHaplotypes);
					_currentComponent = tableForHaplotypes;
				
				/*} catch (GDMSException e) {
					//e.printStackTrace();
					return;
				}*/
				_buildRightSideUploadLayout.setMargin(false, true, true, true);
			}else{	
				...
				//System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  _strItemSelected :"+_strItemSelected);
				try {
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (event.getSource() == _treeForGDMSUpload){
			...
			if (false == _treeForGDMSUpload.hasChildren(openedItemId)){
				...
				//System.out.println("....................  if condition,,,,,,,"+_strItemSelected);
				if (null != _strItemSelected || false == _strItemSelected.equals("")){
					
					if(_strItemSelected.equalsIgnoreCase("Haplotypes")){
						//System.out.println("if haplotypes seleted");
						
						UploadHaplotypesComponent uploadHaplotypesComponent = new UploadHaplotypesComponent(_mainHomePage, _strItemSelected);
						HorizontalLayout tableForHaplotypes;
						//try {
							tableForHaplotypes = uploadHaplotypesComponent.buildTabbedComponentForHaplotypes();
							_buildRightSideUploadLayout.replaceComponent(_currentComponent, tableForHaplotypes);
							_currentComponent = tableForHaplotypes;
				...
				//strSelectedNode =event.getItemId().toString();
			}
		}
