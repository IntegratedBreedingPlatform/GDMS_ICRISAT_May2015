...
public class UploadMarkerInformationComponent  implements Component.Listener {
	...
	private boolean bUploadDataDirectly;
	final String str1="";
	String pathWB="";
	 String instDir="";
			...
			public void buttonClick(ClickEvent event) {
				...
				if (0 == iSelectedTab){
					...
					for (int i = 1; i < listOfDataColumnFields.size(); i++){
						//System.out.println("%%%%%%%%%%%%%%%%%%   :"+listOfDataColumnFields.get(i));
...
*/
		...
		gdmsFileChooser.registerListener(new FileUploadListener() {
			public void updateLocation(String absolutePath) {
				...
				try {
					...
					/*String strop=uploadMarker.checkDataSize(_mainHomePage);
					if(!strop.equals("no")){
						bUploadDataDirectly = true;
						_mainHomePage.getMainWindow().getWindow().showNotification(strop, Notification.TYPE_ERROR_MESSAGE);
						//break;
						return;				
					}
					*/
					...
					String strop=uploadMarker.validateDataInExcelSheet();
					if(!strop.equals("no")){
						bUploadDataDirectly = true;
						_mainHomePage.getMainWindow().getWindow().showNotification(strop, Notification.TYPE_ERROR_MESSAGE);
						//break;
					...
					System.out.println("%%%%%%%%%%%%%%%%%%%% ");
					uploadMarker.createObjectsToBeDisplayedOnGUI(_mainHomePage);

					sourceDataListToBeDisplayedInTheTable = uploadMarker.getDataFromSourceSheet();
					//System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^   listOfDataColumnFields=:"+listOfDataColumnFields.size());
					if (null != listOfDataColumnFields){
		...
		//20131206: Tulasi --- Added the condition to retrieve maps only for Mapping Data type
				if (_strMarkerType.equalsIgnoreCase("Allelic Data") || _strMarkerType.equalsIgnoreCase("ABH Data")) {
					final List<String> listOfAllMaps = retrieveAllMaps();
					if (null != listOfAllMaps){
						for (int i = 0; i < listOfAllMaps.size(); i++){
							//Map map = listOfAllMaps.get(i);
							comboBoxForMap.addItem(listOfAllMaps.get(i));
						}
					}
					...
					comboBoxForMap.addListener(new Property.ValueChangeListener() {
						...
						public void valueChange(ValueChangeEvent event) {
							...
							if (mapValue instanceof Integer){
								...
								if (null != listOfAllMaps){
									for (int i = 0; i < listOfAllMaps.size(); i++){
										String map = listOfAllMaps.get(i);
										if (map.equals(strMapSelected)){
											bIsValidMap = true;
											break;
										}
									}
	...
	//	txtFieldForDatasetName.se
		...
		//System.out.println("_strMarkerType:"+_strMarkerType);
		if (_strMarkerType.equalsIgnoreCase("SSR Genotype") || _strMarkerType.equalsIgnoreCase("LGC Genomics SNP") || _strMarkerType.equalsIgnoreCase("Generic SNP") || _strMarkerType.equalsIgnoreCase("DArt Genotype") || _strMarkerType.equalsIgnoreCase("Allelic Data") || _strMarkerType.equalsIgnoreCase("ABH Data")){
			topHorizontalLayout.addComponent(comboBoxForGermplasm);
		}
	...
	private ArrayList<String> retrieveAllGermplasms() {
		...
		try{
			//System.out.println("CENTRAL............."+list.getAllGermplasmLists(0, (int)list.countAllGermplasmLists(), Database.CENTRAL));
			List<GermplasmList> resCen=list.getAllGermplasmLists(0, (int)list.countAllGermplasmLists(), Database.CENTRAL);
			for(GermplasmList resList:resCen){
				if(resList.getType().equalsIgnoreCase("LST"))
					listOfGermplasmLists.add(resList.getName());
			}
		}catch (Exception e){
			e.printStackTrace();
		}
		...
		try{
			...
			//System.out.println("LOCAL---------------"+list.getAllGermplasmLists(0, (int)list.countAllGermplasmLists(), Database.LOCAL));
			List<GermplasmList> resLoc=list.getAllGermplasmLists(0, (int)list.countAllGermplasmLists(), Database.LOCAL);
			for(GermplasmList resListL:resLoc){
				if(resListL.getType().equalsIgnoreCase("LST"))
					listOfGermplasmLists.add(resListL.getName());
	...
	private List<String> retrieveAllMaps() throws GDMSException {
		...
		List<String> listOfAllMaps = new ArrayList<String>();
		try {
			
			//System.out.println("...:"+genoManager.getAllMaps(0, (int)genoManager.countAllMaps(Database.CENTRAL), Database.CENTRAL));
			...
			if (null != listOfAllMapsLocal) {
				for (Map map : listOfAllMapsLocal){
					if (false == listOfAllMaps.contains(map.getMapName())){
						listOfAllMaps.add(map.getMapName());
					}
				}
			}
...
*/
	...
	protected void buildListOfDataObjectsToBeUploaded() {
		...
		if (strSourceSheetTitle.equalsIgnoreCase("SSRMarkers")){
			...
			if (valueGermplasm instanceof Integer){
				...
				if (itemId.equals(1)){
					_mainHomePage.getMainWindow().getWindow().showNotification("Please select the Germplasm List required for SNP upload.", Notification.TYPE_ERROR_MESSAGE);
					return;
				} 
			}
			...
			if (valueGermplasm instanceof Integer){
				Integer itemId = (Integer)valueGermplasm;
				if (itemId.equals(1)){
					_mainHomePage.getMainWindow().getWindow().showNotification("Please select the Germplasm List required.", Notification.TYPE_ERROR_MESSAGE);
					return;
				} 
			}
	...
	private void uploadDataDirectly() {
		
		if (strSourceSheetTitle.equalsIgnoreCase("LGCGenomicsSNPGenotype_Source")){
		...
		_gdmsModel.setGermplasmSelected(valueGermplasm.toString());
		String strMarkerType = _strMarkerType.replace(" ", "");
		if(!strMarkerType.equalsIgnoreCase("GenericSNP")){
			try{
			//strMarkerType.equals("GenericSNP")
			
				//System.out.println("%%%%%%%%%%%%%%%%%%%% *************"+strMarkerType);
				uploadMarker.createObjectsToBeDisplayedOnGUI(_mainHomePage);
			} catch (GDMSException e) {
				_mainHomePage.getMainWindow().showNotification(e.getExceptionMessage(), Notification.TYPE_ERROR_MESSAGE);
				return;
		...
		//System.out.println("Objects created...............");
			sourceDataListToBeDisplayedInTheTable = uploadMarker.getDataFromSourceSheet();
			//System.out.println("^^.^^.^^.^^.^^.^^^.^^.^^^.^^^.^^^   listOfDataColumnFields=:"+sourceDataListToBeDisplayedInTheTable);
			if (null != listOfDataColumnFields){
				dataListToBeDisplayedInTheTable = uploadMarker.getDataFromDataSheet();
			}
		...
		//System.out.println("dataListToBeDisplayedInTheTable:"+dataListToBeDisplayedInTheTable);
		
		
		if (listOfSourceColumnFields.size() > 0){
			uploadMarker.setListOfColumns(listOfSourceColumnFields);
			uploadMarker.setDataToBeUploded(sourceDataListToBeDisplayedInTheTable, dataListToBeDisplayedInTheTable, dataListFromAdditionalGIDsSheetForDArTGenotype);
			...
			try {
				...
				strMarkerType = _strMarkerType.replace(" ", "");
				if (strMarkerType.equals("SNPGenotype") || strMarkerType.equals("GenericSNP") || strMarkerType.equals("LGCGenomicsSNP")) {
	...
	private void getMarkerFields() {
		...
		if (strMarkerType.equals("SSRMarker")){
			...
			strSourceSheetTitle = "MTA_Source";
			strDataSheetTitle = "MTA_Data";
		}else {
			listOfSourceFieldProperties = UploadTableFields.SSRMarker;
		}
	...
	private Layout buildDataTableComponent() {
		...
		if (null != dataListToBeDisplayedInTheTable && 0 != dataListToBeDisplayedInTheTable.size()){
			 HashMap<String,String> hashMap1 = dataListToBeDisplayedInTheTable.get(0);
			 /*if (hashMap1.size() > 20) {
			    	System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
				     bUploadDataDirectly = true;
		...
		/*if (null != listOfDataColumnFields && listOfDataColumnFields.size() > 0){
			if (200 < listOfDataColumnFields.size()){
				bUploadDataDirectly = true;
				String strErrorMessage = "Data to be uploaded cannot be displayed in the table." + 
			                              " Please click on upload to upload the data directly";
				_mainHomePage.getMainWindow().getWindow().showNotification(strErrorMessage, Notification.TYPE_ERROR_MESSAGE);
