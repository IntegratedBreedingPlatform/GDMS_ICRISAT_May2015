...
public class RetrieveComponent extends CustomComponent implements ItemClickListener  {
	...
	//20131209: Tulasi --- Implemented code to display Map and QTL tables
		...
		private Table getMapTableComponent() {
			...
			if(!finalretrieveMarker2.isEmpty()) {
	...
	private VerticalLayout buildRetrieveTreeComponent() {
		...
		final Object[][] uploadTemplates = new Object[][]{
				new Object[]{"Genotyping Data", "Germplasm Names", "Markers", "GID", "Dataset"},
				//new Object[]{"Genotyping Dataset"},
				//new Object[]{"Polymorphic Markers"},
				new Object[]{"Marker Information"},
				new Object[]{"Maps/QTLs", "Map", "QTL", "Trait"},
				new Object[]{"Search", "Polymorphic Markers", "Haplotype/Allele", "Region between flanking markers"} 
		};
...
//						treeForGDMSRetrieve.setParent("Polymorphic Markers", childNode);	
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (_strItemSelected.equals("Marker Information")){
			...
			_buildRightSideRetrieveLayout.replaceComponent(currentComponent, buildTableComponent);
			currentComponent = buildTableComponent;
		}else if (_strItemSelected.equals("Haplotype/Allele")){
			RetrieveHaplotypesComponent retrieveHapComponent = new RetrieveHaplotypesComponent(_mainHomePage);
			HorizontalLayout tabbedComponentForMarkers = retrieveHapComponent.buildTabbedComponentForMarkers();
			_buildRightSideRetrieveLayout.replaceComponent(currentComponent, tabbedComponentForMarkers);
			currentComponent = tabbedComponentForMarkers;
		}else if (_strItemSelected.equals("Region between flanking markers")){
			RetrieveRegionBetweenFlankingMarkersComponent retrieveRegionComponent = new RetrieveRegionBetweenFlankingMarkersComponent(_mainHomePage);
			HorizontalLayout tabbedComponentForMarkers = retrieveRegionComponent.buildTabbedComponentForFlankingMarkers();
			_buildRightSideRetrieveLayout.replaceComponent(currentComponent, tabbedComponentForMarkers);
			currentComponent = tabbedComponentForMarkers;
		}
