...
public class RetrieveMarkerInformationComponent_OLD implements Component.Listener {
	...
	/*private MarkerInfoDAO markerInfoDAOLocal;
	private MarkerInfoDAO markerInfoDAOCentral;*/
	...
	public RetrieveMarkerInformationComponent_OLD(GDMSMain theMainHomePage) throws GDMSException{
	...
	private Component buildMarkerConditionsComponent() {
		...
		final String[] strArrayOfFields = {"Marker-Name", "Marker-Type"};
		...
		String[] containsFields = new String[] {"Contains", "Equals", "Not Equals"};
		...
		btnAdd.addListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(strMarkerField.equalsIgnoreCase("Marker-Type") && strCondition.equalsIgnoreCase("contains")){					
					_mainHomePage.getMainWindow().getWindow().showNotification("Condition Contains not allowed for property Marker-Type",  Notification.TYPE_ERROR_MESSAGE);
					return;
				}
...
*/									genoManager=factory.getGenotypicDataManager();
	...
	private Table[] buildMarkerTable() throws GDMSException, MiddlewareQueryException {		
		...
		for (int i = 0; i < listOfSelectedCriteria.size(); i++){
			...
			hmGenotypeCount=new HashMap<Integer, BigInteger>();
			List<MarkerInfo> gCount=genoManager.getMarkerInfoByMarkerIds(_markerIDsList);
			for(MarkerInfo resGCount:gCount){
				if(resGCount.getGenotypesCount()!=null)
					hmGenotypeCount.put(resGCount.getMarkerId(), resGCount.getGenotypesCount());
			}
			...
			if (propertyFieldName.toString().equals("Marker-Type")){				
					...
					if (strCondition.equals("Equals")){
						...
						_mainHomePage.getMainWindow().getWindow().showNotification("Operation Contains not allowed for property Marker-Type",  Notification.TYPE_ERROR_MESSAGE);						
					}
				//}
			...
			for (int iTableCntr = 0; iTableCntr < arrayOfTables.length; iTableCntr++) {
				...
				for (int i = 0; i < _finalListOfMarkers.size(); i++){
					...
					if (null != markerUserInfo){
						...
						contact = markerUserInfo.getContactValue();
					...
					if(hmGenotypeCount.containsKey(Integer.parseInt(markerId.toString()))){
						genotypeCount=hmGenotypeCount.get(Integer.parseInt(markerId.toString()))+"";
					}else
						genotypeCount="";
					...
					gCount=genotypeCount+"";
					
					Button genotypeCountLink = new Button();
					//if(!gCount.isEmpty()){
						genotypeCountLink.setCaption(gCount);
						genotypeCountLink.setStyleName(Reindeer.BUTTON_LINK);
						genotypeCountLink.setDescription(gCount);
