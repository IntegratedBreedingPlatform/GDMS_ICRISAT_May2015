...
public class DataDeletionAction {
	...
	public boolean deleteMTAInfoData(List<String> itemsToDelete) {
		...
		try {
			/*factory=GDMSModel.getGDMSModel().getManagerFactory();
			genoManager=factory.getGenotypicDataManager();*/
			String strDisplayMessage = "Successfully deleted following MTA(s) \n\n";
			int iCount = 1;
			for (String string : itemsToDelete) {
				...
				try {
					genoManager.deleteMTA(datasetIds);
				} catch (Exception e) {
			...
			GDMSInfoDialog gdmsMessageWindow = new GDMSInfoDialog(_mainHomePage, emptyList , strDisplayMessage);
	...
	public boolean deleteQTLInfoData(List<String> itemsToDelete) {
		...
		try {
			/*factory=GDMSModel.getGDMSModel().getManagerFactory();
			genoManager=factory.getGenotypicDataManager();*/
			String strDisplayMessage = "Successfully deleted following QTL(s) \n\n";
			int iCount = 1;
			for (String string : itemsToDelete) {
				strDisplayMessage += iCount++ + ". "+ string +"\n";
				List<DatasetElement> detailsByName=genoManager.getDatasetDetailsByDatasetName(string, Database.LOCAL);
...
//		}
	...
	public boolean deleteGenotypingData(List<String> itemsToDelete) {
		...
		String strDisplayMessage = "Successfully deleted following Genotyping dataset \n\n";
